...
describe('Crew Members API Endpoints', () => {
  ...
  test('GET /api/workspaces/:workspaceId/crew-members should return all crew members for a workspace', async () => {
    const response = await request(app).get(`/api/workspaces/${demoWorkspaceId}/crew-members`);
  ...
  test('GET /api/crew-members/:id should return a specific crew member', async () => {
    ...
    const listResponse = await request(app).get(`/api/workspaces/${demoWorkspaceId}/crew-members`);
    const crewMemberId = listResponse.body[0].id;
    
    const response = await request(app).get(`/api/crew-members/${crewMemberId}`);
  ...
  test('DELETE /api/crew-members/:id should delete a crew member', async () => {
    const response = await request(app).delete(`/api/crew-members/${createdCrewMemberId}`);
    
    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty('success', true);
    
    // Verify it's deleted
    const verifyResponse = await request(app).get(`/api/crew-members/${createdCrewMemberId}`);
  ...
  describe('Crew Member Jobs Endpoints', () => {
    ...
    test('GET /api/workspaces/:workspaceId/crew-member-jobs should return all crew member jobs', async () => {
      const response = await request(app).get(`/api/workspaces/${demoWorkspaceId}/crew-member-jobs`);
    ...
    test('GET /api/crew-members/:crewMemberId/jobs should return jobs for a crew member', async () => {
      const response = await request(app).get(`/api/crew-members/${crewMemberId}/jobs`);
    ...
    test('DELETE /api/crew-member-jobs/:id should remove a job assignment', async () => {
      const response = await request(app).delete(`/api/crew-member-jobs/${crewMemberJobId}`);
