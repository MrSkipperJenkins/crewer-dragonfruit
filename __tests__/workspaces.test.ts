...
describe('Workspace API Endpoints', () => {
  ...
  test('GET /api/workspaces/:id should return a specific workspace', async () => {
    const response = await request(app).get(`/api/workspaces/${demoWorkspaceId}`);
  ...
  test('DELETE /api/workspaces/:id should delete a workspace', async () => {
    const response = await request(app).delete(`/api/workspaces/${createdWorkspaceId}`);
    
    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty('success', true);
    
    // Verify it's deleted
    const verifyResponse = await request(app).get(`/api/workspaces/${createdWorkspaceId}`);
