...
export function FullCalendarView() {
  ...
  const { getCrewStaffingStatus, crewAssignmentQueries, requiredJobQueries } =
    useShowStaffing(showIds);
  ...
  const calendarEvents = useMemo(() => {
    ...
    categories,
    resources,
    showResourceQueries.data,
    crewAssignmentQueries.data,
    requiredJobQueries.data,
  ]);
  ...
  const renderEventContent = (eventInfo: any) => {
    ...
    const resourcesText =
      ...
      extendedProps.resources !== "No resources assigned"
    ...
    const safeCrewStatus = crewStatus || {
      assigned: 0,
      required: 0,
      isFullyStaffed: false,
    };
    const crewText =
      safeCrewStatus.required > 0
        ? `${safeCrewStatus.assigned} of ${safeCrewStatus.required} crew assigned`
        : "No crew needed";

    const isFullyStaffed =
      safeCrewStatus.isFullyStaffed && safeCrewStatus.required > 0;
    const crewStatusColor = isFullyStaffed
      ? "text-green-200"
      : safeCrewStatus.required > 0
        ? "text-yellow-200"
        : "text-gray-300";
    ...
    return (
      <div className="fc-event-main-frame">
        ...
        <div className="text-xs opacity-90 mt-1 space-y-0.5">
          <div className="truncate">{resourcesText}</div>
          <div className={`${crewStatusColor} font-medium`}>{crewText}</div>
        </div>
      </div>
    );
  };
